---
name: orquestador-vdc-reference
description: Visual Direction Checkpoint completo (Fase 2, Pasos 1.5a/b/c) — extracción polimórfica de referencia (figma/image/url/brand/preset), pre-fill de las 8 decisiones + template de presentación, reglas del checkpoint (anti-loop máx 3), schema extendido del save {proyecto}/visual-direction, y listado exhaustivo de opciones (bloque legacy). Cargado por el orquestador SOLO al llegar a Fase 2 Paso 1.5 en proyectos con UI. Prerequisito: {proyecto}/intent existe. Post-condición: {proyecto}/visual-direction existe con extraction_status seteado. Extraído de orquestador.md el 2026-07-02 para reducir tokens residentes (plan Engram #3521).
---

# Orquestador — Visual Direction Checkpoint (Fase 2, Paso 1.5)

> Detalle completo de los Pasos 1.5a/b/c. El stub con la esencia y el Phase Gate vive en `orquestador.md` § FASE 2 Paso 1.5. Si esto cambia, actualizar el stub en sincronía.
...
| `image` | (1) Copiar/descargar a `.pipeline/references/ref-image.{ext}`. (2) Color quantization: SI existe `~/.claude/design-data/scripts/extract-palette.js` usarlo con `node`; si NO existe (default actual — el script no está creado), el orquestador inspecciona visualmente la imagen y lista 5-7 hex dominantes. Ambos caminos son válidos. (3) Inferir mood tags (warm/cool, light/dark, editorial/minimal/etc.) vía vision. (4) Inferir familia tipográfica si hay texto legible. |
