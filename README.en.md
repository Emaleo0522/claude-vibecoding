...
- **Simplicity First in outputs** (2026-05-26): every response starts with a 1-3 sentence TL;DR that directly answers the question; if the natural answer ends there, it ends there. Escalates to structure (tables, sections) only if you asked for analysis/comparison/plan or there are ≥3 comparable items with parallel fields. Drops to plain prose if you asked for "summary", "short", "quick", "in simple words". Anti-patterns: `##` headers to look structured in short answers, tables with 2 rows, starting with analysis before the direct conclusion. Details: [`agents/simplicity-first-reference.md`](agents/simplicity-first-reference.md).
- **Architecture Decision Records (ADR) in Engram** (2026-05-24): every time an external pattern/paradigm (paper, library, third-party system) is analyzed vs the vibecoding system, the decision is persisted in Engram with prefix `claude-vibecoding/architecture-review/{YYYY-MM-DD}-{slug}`. States: ADOPTED / DEFERRED / REJECTED / PARTIAL MITIGATION. The `self-auditor` T9 reads these observations and reports drift (adopted decision whose change disappeared), overdue (re-evaluation due), or reconsidered (rejected pattern reappearing). Avoids re-analyzing the same thing + detects when something adopted got silently undone.
...
- All cross-PC `mem_save` must use `scope="personal"` + explicit `project=`. The MCP auto-detect routes to different buckets depending on the client's cwd and breaks the cross. See the complete "save in engram" protocol in [`CLAUDE.md`](CLAUDE.md).
...
In **normal mode**, Claude responds as always but doesn't work bare-handed: beyond hooks, AUTO_AUDIT and memory, it reaches by reflex for the same sub-agents and references as the pipeline when the task warrants it —an `Explore` to understand code without flooding context, a specialized agent for auth/SEO/deploy, a technical reference before heavy work—, without asking permission to use them. In **orchestrator mode**, it takes the coordinator role and delegates to the 24 sub-agents. In **modification mode**, it runs a mini-pipeline (Step 0 audit → light planning → dev+QA only of affected agents). In **diagnostic mode** it's read-only by doctrine: only reads and returns a structured report with findings by severity — useful for auditing projects not generated by this system (due diligence, code review of external repos).
...
| [`CLAUDE.md`](CLAUDE.md) | The `CLAUDE.md` that gets installed — all system doctrine (Human Checkpoint, Engram, hooks, mod mode) |
...
2. If you add a new hook, add it to the table in `CLAUDE.md` (repo root, single source of truth since 2026-05-26) and the `install/linux.sh` script.
